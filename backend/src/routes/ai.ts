...
function isRetryableError(err: any): boolean {
  ...
  // If it's a known non-retryable error type from the SDK
...
router.post('/generate', authenticate, async (req: AuthRequest, res: Response) => {
  const { tool, input, tone, creativity, wordLimit } = req.body;
  ...
  try {
    ...
    res.json({ output, creditsDeducted: user.role === 'internal' ? 0 : cost });
  } catch (err: any) {
